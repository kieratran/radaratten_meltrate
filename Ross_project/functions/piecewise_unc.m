% piecewise attenuation rate uncertainty
function atten_unc=piecewise_unc(geo_power,depth,surface_pick,bed_pick,segment_length,percentage)
...
if abs(depth(bed_pick)-depth(surface_pick))/segment_length >= 1 % ice thickness must be greater than segment length
    ...
    while ending >= depth(bed_pick) % keep sliding the window until segment is smaller than the window
        index=depth<=(starting+5e-5)&depth>=(ending-5e-5); % +/- 0.00005 for expanding the range
        atten_rate(index)=slopeSE(depth(index), geo_power(index));
        n=n+1;
        starting=ending;
        ending=starting-segment_length;
        %plot(polyval(p,depth(index)),depth(index),'r','LineWidth',1.5); hold on;
    end
...
atten_unc=atten_rate/2*1000; % convert from two- way dB/m to one-way dB/km
